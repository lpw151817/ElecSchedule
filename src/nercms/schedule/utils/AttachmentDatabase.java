...
public class AttachmentDatabase {
		...
		try {
			...
			while (false == cursor.isAfterLast()) {
				for (int i = 0; i < columns_num; ++i) {
					map.put(cursor.getColumnName(i) + "_" + Integer.toString(index),
							cursor.getString(i));
	...
	public class EvidenceDatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			/*
			 * db.execSQL(CREATE_GPS_TABLE_SQL); Log.v(LOG_TAG,
			 * "SQLite: onCreate1"); db.execSQL(CREATE_EVIDENCE_TABLE_SQL);
			 * Log.v(LOG_TAG, "SQLite: onCreate2");
			 * db.execSQL(CREATE_PHONE_NUMBER_TABLE_SQL); Log.v(LOG_TAG,
			 * "SQLite: onCreate3"); db.execSQL(CREATE_LOG_TABLE_SQL);
			 * Log.v(LOG_TAG, "SQLite: onCreate4");
			 * db.execSQL(CREATE_ORGNIZATION_TABLE_SQL); Log.v(LOG_TAG,
			 * "SQLite: onCreate5"); db.execSQL(CREATE_GROUP_TABLE_SQL);
			 * Log.v(LOG_TAG, "SQLite: onCreate6");
			 * db.execSQL(CREATE_RESOURCE_TABLE_SQL); Log.v(LOG_TAG,
			 * "SQLite: onCreate7");
			 * db.execSQL(CREATE_RESOURCE_POSITION_TABLE_SQL); Log.v(LOG_TAG,
			 * "SQLite: onCreate7");
			 */
		}
